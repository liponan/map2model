...
def aa3toaa1(x):
    table = dict(ala="A", arg="R", asn="N", asp="D", asx="B", cys="C", gln="Q", glu="E", glx="Z",
                 ...
                 thr="T", trp="W", tyr="Y", val="V", ukn="x", unk="x", sec="x")
    if isinstance(x, str):
        return table[x.lower()]
    if isinstance(x, list):
        return [table[i.lower()] for i in x]
    if isinstance(x, np.ndarray):
        return np.array([table[i.lower()] for i in x])
...
def aa1toaa3(x):
    ...
    if isinstance(x, np.ndarray):
        return np.array([table[i.lower()].upper() for i in x])


def aa1toidx(x):
    table = dict(A=0, R=1, N=2, D=3, C=4, Q=5, E=6, G=7, H=8, I=9, L=10, K=11, M=12, F=13, P=14, S=15, T=16, W=17, Y=18,
                 V=19, X=20)
    if isinstance(x, str):
    ...
    if isinstance(x, np.ndarray):
        return np.array([table[i.upper()] for i in x])
...
def idxtoaa1(idx):
    ...
    if isinstance(idx, np.ndarray):
        return np.array([table[i] for i in idx])
...
def rot_to_idx(rot_label):
    ...
    if isinstance(rot_label, np.ndarray):
        return np.array([table[i.lower()] for i in rot_label])


def rot_to_aa_idx(rot_label):
    if isinstance(rot_label, str):
        return aa1toidx(aa3toaa1(rot_label[0:3].lower()))
    if isinstance(rot_label, list):
        return aa1toidx(aa3toaa1([i[0:3].lower() for i in rot_label]))
    if isinstance(rot_label, np.ndarray):
        return aa1toidx(
            aa3toaa1(
                np.array([i[0:3].lower() for i in rot_label])
            )
        )
